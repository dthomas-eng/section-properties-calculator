...
/*
if(document.referrer == "https://visualcalcs.com/spclite/" || document.referrer == "http://visualcalcs.com/spclite/" || document.referrer == "https://visualcalcs.com" || document.referrer == "http://visualcalcs.com") {
    //go right ahead, sir...
}
else{
...
function createNewLineSnip(linestartx, linestarty, lineendx, lineendy, constLine){
	...
	linearDimArray.push(new LinearDimension(actLength, dimXpos, dimYpos, ElementID, false,dimlineXstart, dimlineYstart, dimlineXend, dimlineYend,orientation,0,0,0,0,0,0,0,0,0,angle,xoffset1, yoffset1, xoffset2, yoffset2, perpOffset));
...
//DOES THIS CALL deletedim at some point?
...
function suggestAngleSnaps(startx,starty,baseLineID){
    ...
    for(var i = 0; i < snapLineData.length; i++){
		...
		if((closeness < .1) && (ControlDown == false)){
        	snapAngle = sl[4];
		}
    }
...
//    var InfoString = CurrentFileName + ' |' + dateTime + '\n|' + Areastr +' |' + Cxstr +' |' + Cystr +'\n|' + Ixxstr +' |' + Iyystr +' |' + Thetastr +' |' + Ixpstr +' |' + Iypstr;
    var InfoString = 'Displaying Results For:\n' +CurrentFileName + '\n' + dateTime;
    ...
    var ResultsString = 'Area:   ' + MasterAreaStr + '\nCX:     ' + actualCxStr + '\nCY:     ' + actualCyStr + '\nIX:     ' + MasterIxxStr + '\nIY:     ' + MasterIyyStr + '\nTheta:  ' + MasterAlphaStr + '\nIp1:    ' + MasterIxpStr + '\nIp2:    ' + MasterIypStr;
...
function UtilityPointDraw(callback){
    ...
    if(inboundsCheck() == true){
    	...
    	ourRequest.open('POST', 'https://visualcalcs.pythonanywhere.com/L489UIndiasII_d');
...
//This function passes anything that's passed into it into the server log.
function PrintToLog(datatoprint){

	var ourRequest = new XMLHttpRequest();

	ourRequest.open('POST', 'https://visualcalcs.pythonanywhere.com/PrintToLog');
	ourRequest.setRequestHeader("Content-type", "text/plain");
